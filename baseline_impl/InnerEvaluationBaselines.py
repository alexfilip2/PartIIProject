...
def generate_splits_baseline(baseline_config: ConfigBaselines, data: dict, outer_flag=True) -> tuple:
    '''
...
def evaluate_baseline(baseline_config: ConfigBaselines) -> dict:
    '''
    ...
    if results:
        ...
        print('Evaluating the baseline model %s...' % baseline_config)
    # load the data
...
def inner_nested_cv_baselines(baseline_name: str) -> tuple:
    '''
    ...
    grid = ParameterGrid(ConfigBaselines.get_sampled_models(baseline_name))
    for eval_out in range(ConfigBaselines().params['k_outer']):
        inner_results[eval_out] = {}
        for hyper_params in grid:
            for eval_in in range(ConfigBaselines().params['k_inner']):
                # create the configuration object for the baseline
                config = ConfigBaselines(hyper_params)
...
def inner_losses_baseline(baseline_name: str, filter_by_params: dict = None):
    '''
