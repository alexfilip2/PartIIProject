...
def attn_head(input_feat_seq, out_size, adj_mat, bias_mat,
    ...
    """ Builds a single attention head of the GAT architecture

        Parameters
        ----------
        input_feat_seq : tensor of shape (1, nb_nodes, feat_vect_length)
            The tensor storing the per-node feature vectors for all the nodes in an example graph
        out_size : int
            The dimensionality of the new node feat. vecs generated by this attention head
        adj_mat : tensor of shape (1, nb_nodes, nb_nodes)
            The tensor storing the adjacency matrix of the example graph with edge weights (not binary adj)
        bias_mat : tensor of shape (1, nb_nodes, nb_nodes)
            The tensor storing a bias matrix used for MASKED ATTENTION: discard the non-neighbours from the aggregation
        activation : function
            The activation function of the head applied to the feature vectors before output them
        include_weights : bool
            Flag for including or not the EDGE WEIGHTS when generating the alpha_ij coefficients
        ...
        Returns
        -------
        ret : tensor of shape (1, nb_nodes, out_size)
            The tensor storing the new feature vectors for all the nodes in the initial graph
        """
    ...
    with tf.name_scope('my_attn'):
        ...
        # add a bias to the plain result
        ...
        return activation(ret)  # apply activation of the attention head
