...
# load the STRUCTURAL ADJACENCY MATRICES
def get_structural_adjs():
    print(dir_root_structural_data)
    adjs_binary = os.path.join(dir_proc_struct_data, 'adjs_matrices.pkl')
...
# load the STRUCTURAL NODE FEATURES
def get_scaled_struct_node_feat():
    node_feats_binary = os.path.join(dir_proc_struct_data, 'node_feats.pkl')
...
def load_struct_data(hyparams):
    ...
    dataset_binary = os.path.join(dir_proc_struct_data, binary_prefix)
