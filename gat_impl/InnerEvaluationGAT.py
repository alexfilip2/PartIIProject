...
def generate_splits_gat(gat_config: ConfigGAT, data: dict, outer_flag=True) -> tuple:
    '''
...
def evaluate_gat(config: ConfigGAT) -> dict:
    '''
...
def inner_nested_cv_gat():
    ...
    lookup_table = {}
    inner_results = {}
    grid = ParameterGrid(ConfigGAT.get_sampled_models())
    for eval_out in range(ConfigGAT().params['k_outer']):
        ...
        for hyper_params in grid:
            ...
            for eval_in in range(ConfigGAT().params['k_inner']):
                gat_model_config = ConfigGAT(hyper_params)
...
def inner_losses_gat(filter_by_params: dict = {}):
    ...
    if os.path.exists(inner_losses_file):
        ...
        with open(inner_losses_file, 'wb') as fp:
            pkl.dump((inner_results, lookup_table), fp)
